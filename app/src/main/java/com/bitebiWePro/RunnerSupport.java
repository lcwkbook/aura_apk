...
public final class RunnerSupport {
  ...
  private void reportFileStatus(String dashUrl, String filePath, String deviceId) {
    new Thread(
            () -> {
              try {
                ...
                if (!file.exists()) {
                  // 文件不存在 → 上报缺失
                  URL url = new URL(dashUrl + "/api.php?action=report_file_missing");
                ...
                // 上报文件状态
                URL url = new URL(dashUrl + "/api.php?action=report_file");
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestMethod("POST");
                conn.setRequestProperty("Content-Type", "application/json");
                conn.setDoOutput(true);
                ...
                Log.e("FILE_MONITOR", "上报文件状态失败: " + e.getMessage());
              }
            })
        .start();
  }
  ...
  public static File copyFromPath(Context context, File source, String originalName)
      throws Exception {
    if (source == null) throw new Exception("运行文件为空");
    String name =
        displayFileName(
            originalName == null || originalName.length() == 0 ? source.getName() : originalName);
    File base = Build.VERSION.SDK_INT >= 21 ? context.getCodeCacheDir() : context.getFilesDir();
    File dir = new File(base, "runner");
    if (!dir.exists() && !dir.mkdirs()) {
      throw new Exception("无法创建临时运行目录: " + dir.getAbsolutePath());
  ...
  public static void chmod777(File file) {
    ...
    try {
      Process p =
          new ProcessBuilder("/system/bin/sh", "-c", "chmod 777 " + quote(file.getAbsolutePath()))
              .start();
      waitFor(p, 2500);
    } catch (Exception ignored) {
    }
  ...
  public static String prepareRootExecutable(File localFile, String originalName) throws Exception {
    ...
    String cmd =
        "mkdir -p "
            ...
            + quote(tempPath)
            + " && cat "
            + quote(localFile.getAbsolutePath())
            + " > "
            + quote(tempPath)
            + " && chmod 777 "
            + quote(tempPath)
  ...
  public static String prepareRootExecutableFromPath(String sourcePath, String originalName)
      throws Exception {
  ...
  private static String buildElfBody(String q) {
    return q
        ...
        + "if [ -x /system/bin/linker64 ]; then /system/bin/linker64 "
        + q
        + "; exit $?; "
        + "elif [ -x /system/bin/linker ]; then /system/bin/linker "
        + q
        + "; exit $?; "
        + "else exit $code; fi; "
        + "else exit $code; fi";
  ...
  public static String buildDetachedFileCommand(File file, boolean isElf) {
    ...
    if (isElf) {
      ...
      body = "if command -v node >/dev/null 2>&1; then node " + q + "; else exit 127; fi";
    } else {
      body = q;
    }
    File parent = file.getParentFile();
  ...
  public static String buildInteractiveTerminalCommand(File file, boolean isElf) {
    ...
    String shellBody =
        buildTouchCompatibilityCommand(null)
            + "export TERM=xterm; export HOME=/data/local/tmp; export TMPDIR=/data/local/tmp;"
            + " export"
            + " PATH=/system/bin:/system/xbin:/vendor/bin:/vendor/xbin:/sbin:/data/local/tmp:$PATH;"
            + " "
            + body;
  ...
  public static String buildRootShellCommand(File file, boolean isElf) {
    ...
    String path = file.getAbsolutePath();
    String script =
        "echo '[AuraKernel] Root shell connected'; "
            + "echo '[AuraKernel] Executing: "
            + shellEchoEscape(path)
            + "'; "
            + terminal
  ...
  private static String buildTouchCompatibilityCommand(String packageName) {
    String cmd =
        "settings put global block_untrusted_touches 0 >/dev/null 2>&1 || true; "
    ...
    if (packageName != null && packageName.length() > 0) {
      cmd +=
          "if command -v appops >/dev/null 2>&1; then "
              + "appops set "
              + quote(packageName)
              + " SYSTEM_ALERT_WINDOW allow >/dev/null 2>&1 || true; "
              + "appops set "
              + quote(packageName)
              + " SYSTEM_ALERT_WINDOW 0 >/dev/null 2>&1 || true; "
              + "fi; ";
    }
    return cmd;
  }
  ...
  public static String findMatchingDriverEntryName(File zipFile) throws Exception {
    ...
    try {
      ...
      while ((entry = zip.getNextEntry()) != null) {
        ...
        if (lower.contains(kernel.toLowerCase())
            || normalized.contains(nk)
            || (nkm.length() > 0 && normalized.contains(nkm))) {
  ...
  public static String installDriverFromZip(Context context, File zipFile) throws Exception {
    ...
    CommandResult result =
        runSuCommand(buildDriverInstallCommand(new File(rootPath), isElf), 20000);
    if (result.exitCode != 0) {
  ...
  private static File extractDriverEntry(Context context, File zipFile, String entryName)
      throws Exception {
    File base = Build.VERSION.SDK_INT >= 21 ? context.getCodeCacheDir() : context.getFilesDir();
  ...
  private static File extractDriverEntry(Context context, Uri zipUri, String entryName)
      throws Exception {
    File base = Build.VERSION.SDK_INT >= 21 ? context.getCodeCacheDir() : context.getFilesDir();
    File dir = new File(base, "driver");
    if (!dir.exists() && !dir.mkdirs()) throw new Exception("无法创建驱动临时目录");
  ...
  private static CommandResult runShellCommand(String command, int timeoutMs) throws Exception {
    Process p =
        new ProcessBuilder("/system/bin/sh", "-c", command).redirectErrorStream(true).start();
    StreamCollector collector = new StreamCollector(p.getInputStream());
    collector.start();
    boolean finished = waitFor(p, timeoutMs);
    if (!finished) {
