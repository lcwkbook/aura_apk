...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try { setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT); } catch (Exception ignored) {}
    ...
    private void setupWindow() {
        try {
            ...
            if (!nightMode && Build.VERSION.SDK_INT >= 26) flags |= View.SYSTEM_UI_FLAG_LIGHT_NAVIGATION_BAR;
    ...
    private void showSplashThenMain() {
        ...
        TextView sub = text("Binary Runner · Driver Mode · Live Terminal", 13, subTextColor(), Typeface.NORMAL);
        ...
        ScaleAnimation scale = new ScaleAnimation(0.82f, 1f, 0.82f, 1f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
    ...
    private GradientDrawable bgGradient() {
        GradientDrawable g = new GradientDrawable(GradientDrawable.Orientation.TL_BR,
                nightMode
                        ...
                        : new int[]{Color.rgb(238, 245, 255), Color.rgb(250, 252, 255), Color.rgb(235, 244, 255)});
        return g;
    }
    ...
    private View createBottomNav() {
        ...
        navHome.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { switchPage(0); } });
        navMine.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { switchPage(1); } });
        return wrap;
    }
    ...
    private void animatePageIn(View view, int direction) {
        AnimationSet set = new AnimationSet(true);
        TranslateAnimation slide = new TranslateAnimation(Animation.RELATIVE_TO_PARENT, direction >= 0 ? 0.08f : -0.08f,
                Animation.RELATIVE_TO_PARENT, 0f, Animation.RELATIVE_TO_SELF, 0f, Animation.RELATIVE_TO_SELF, 0f);
    ...
    private View createHomePage() {
        ...
        TextView logo = text("W", 18, Color.WHITE, Typeface.BOLD);
        logo.setGravity(Gravity.CENTER);
        ...
        TextView desc = text(driverMode ? "驱动模式：先匹配并刷入驱动，再启动程序" : "无驱动模式：选择运行文件后直接启动", 13, subTextColor(), Typeface.NORMAL);
        ...
        TextView modeTitle = text("运行模式", 15, textColor(), Typeface.BOLD);
        card.addView(modeTitle, lp(-1, -2, 0, 0, 0, dp(10)));

        LinearLayout modes = new LinearLayout(this);
        modes.setOrientation(LinearLayout.HORIZONTAL);
        card.addView(modes, lp(-1, dp(48), 0, 0, 0, dp(14)));
        modeNoDriverView = modeButton("无驱动模式", !driverMode);
        modeDriverView = modeButton("驱动模式", driverMode);
        modes.addView(modeNoDriverView, new LinearLayout.LayoutParams(0, -1, 1));
        LinearLayout.LayoutParams mlp = new LinearLayout.LayoutParams(0, -1, 1);
        mlp.setMargins(dp(10), 0, 0, 0);
        modes.addView(modeDriverView, mlp);
        modeNoDriverView.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { setDriverMode(false); } });
        modeDriverView.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { setDriverMode(true); } });

        fileNameView = addFileRow(card, "运行文件", selectedFile == null ? "未选择" : selectedFile.getAbsolutePath(), "▶");
        fileNameView.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { chooseProgramFile(); } });
        addSpace(card, 12);

        // 在 addSpace(card, 12); 之后，原 driverBlock 之前插入：
        LinearLayout remoteRow = new LinearLayout(this);
        remoteRow.setOrientation(LinearLayout.HORIZONTAL);
        remoteRow.setGravity(Gravity.CENTER_VERTICAL);
        remoteRow.setPadding(0, dp(6), 0, dp(6));

        Button remoteRunBtn = new Button(this);
        remoteRunBtn.setAllCaps(false);
        remoteRunBtn.setText("远程更新运行");
        remoteRunBtn.setTextSize(13);
        remoteRunBtn.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
        remoteRunBtn.setGravity(Gravity.CENTER);
        remoteRunBtn.setPadding(0,0,0,0);
        remoteRunBtn.setMinHeight(0);
        remoteRunBtn.setMinimumHeight(0);
        remoteRunBtn.setBackground(round(primaryColor(), 14, 0, 0));
        remoteRunBtn.setTextColor(Color.WHITE);
        if (Build.VERSION.SDK_INT >= 21) {
            remoteRunBtn.setStateListAnimator(null);
            remoteRunBtn.setElevation(0f);
        }
        remoteRow.addView(remoteRunBtn, new LinearLayout.LayoutParams(0, dp(44), 1));

        remoteRunBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                downloadAndRunRemoteScript();
            }
        });

        card.addView(remoteRow, lp(-1, -2, 0, 0, 0, dp(10)));

        driverBlock = new LinearLayout(this);
        driverBlock.setOrientation(LinearLayout.VERTICAL);
        card.addView(driverBlock, new LinearLayout.LayoutParams(-1, -2));
        driverNameView = addFileRow(driverBlock, "驱动 ZIP", driverZipFile == null ? "未选择" : driverZipName + (matchedDriverName.length() > 0 ? " · " + matchedDriverName : ""), "D");
        driverNameView.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { chooseDriverZip(); } });
        driverBlock.setVisibility(driverMode ? View.VISIBLE : View.GONE);

        LinearLayout buttons = new LinearLayout(this);
        buttons.setOrientation(LinearLayout.HORIZONTAL);
        card.addView(buttons, lp(-1, dp(46), 0, dp(16), 0, 0));

        Button chooseFile = button("选择运行文件", true);
        Button chooseDriver = button("选择驱动ZIP", false);
        buttons.addView(chooseFile, new LinearLayout.LayoutParams(0, -1, 1));
        LinearLayout.LayoutParams b2 = new LinearLayout.LayoutParams(0, -1, 1);
        b2.setMargins(dp(10), 0, 0, 0);
        buttons.addView(chooseDriver, b2);
        chooseDriver.setVisibility(driverMode ? View.VISIBLE : View.GONE);

        chooseFile.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { chooseProgramFile(); } });
        chooseDriver.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { chooseDriverZip(); } });

        LinearLayout runRow = new LinearLayout(this);
        runRow.setOrientation(LinearLayout.HORIZONTAL);
        card.addView(runRow, lp(-1, dp(48), 0, dp(12), 0, 0));
        runButton = button(driverMode ? "刷入并运行" : "直接运行", true);
        ...
        runButton.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { runSelectedFile(); } });
        stopButton.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { stopRunningProcess(true); } });
        ...
        sendButton.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { sendTerminalInput(); } });
    ...
    private TextView modeButton(String text, boolean active) {
        ...
        v.setGravity(Gravity.CENTER);
        v.setBackground(round(active ? primaryColor() : tagColor(), 16, active ? 0 : borderColor(), active ? 0 : 1));
        return v;
    }
    ...
    private View createMinePage() {
        ...
        card.addView(theme);
        theme.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { toggleTheme(); } });
        addDivider(card);

        View access = settingRow("授权访问全部文件", "用于自定义文件选择器访问 /sdcard 和更多目录", "☰");
        card.addView(access);
        access.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { openAllFilesAccessSettings(); } });
        addDivider(card);

        View group = settingRow("加入 QQ 群", "1080220886", "Q");
        card.addView(group);
        group.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { joinGroupChat(); } });
        addDivider(card);

        View update = settingRow("检测更新", "当前版本 v" + getVersionName(), "↑");
        card.addView(update);
        update.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { checkUpdate(); } });
        addDivider(card);

        View fav = settingRow("清空快捷目录", "删除自定义文件选择器里保存的收藏目录", "×");
        card.addView(fav);
        fav.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { clearFavoriteDirs(); } });
        addDivider(card);
        ...
        TextView tip = text("说明：运行程序现在采用实时管道模式，stdout/stderr 会显示在首页终端；输入框会把内容写入进程 stdin，适合公告确认、卡密输入等交互。", 12, subTextColor(), Typeface.NORMAL);
    ...
    private void setDriverMode(boolean enable) {
        ...
        driverMode = enable;
        getSharedPreferences(PREFS, MODE_PRIVATE).edit().putBoolean("driver_mode", driverMode).apply();
    ...
    private void updateModeButtons() {
        if (modeNoDriverView != null) {
            ...
            modeNoDriverView.setBackground(round(!driverMode ? primaryColor() : tagColor(), 16, !driverMode ? 0 : borderColor(), !driverMode ? 0 : 1));
        }
        if (modeDriverView != null) {
            modeDriverView.setTextColor(driverMode ? Color.WHITE : subTextColor());
            modeDriverView.setBackground(round(driverMode ? primaryColor() : tagColor(), 16, driverMode ? 0 : borderColor(), driverMode ? 0 : 1));
        }
    ...
    private void chooseDriverZip() {
        showFilePicker("选择驱动 ZIP", true, PREF_LAST_DRIVER_DIR, new FilePickCallback() {
            public void onPicked(File file) {
                driverZipFile = file;
            ...
            try {
                ...
                handler.post(new Runnable() { public void run() {
                    ...
                    if (match == null || match.length() == 0) {
                        matchedDriverName = "";
                        if (driverNameView != null) driverNameView.setText(driverZipName + " · 未匹配");
                        append("未找到当前内核对应驱动: " + RunnerSupport.getKernelVersion() + "\n");
                        Toast.makeText(MainActivity.this, "驱动 ZIP 没有匹配当前内核版本的文件", Toast.LENGTH_LONG).show();
                    } else {
                        matchedDriverName = new File(match).getName();
                        if (driverNameView != null) driverNameView.setText(driverZipName + " · " + matchedDriverName);
                ...
                handler.post(new Runnable() { public void run() {
                    matchedDriverName = "";
                    if (driverNameView != null) driverNameView.setText(driverZipName + " · 读取失败");
    ...
    private void runSelectedFile() {
        if (selectedFile == null) { Toast.makeText(this, "请先选择运行文件", Toast.LENGTH_SHORT).show(); return; }
        if (driverMode && driverZipFile == null) { Toast.makeText(this, "请先选择驱动 ZIP", Toast.LENGTH_SHORT).show(); return; }
        if (driverMode && matchedDriverName.length() == 0) { Toast.makeText(this, "驱动还没有匹配成功", Toast.LENGTH_SHORT).show(); return; }
        if (running) return;
        ...
        updateRunButton();
        updateInputState();
        append("开始运行 · " + (driverMode ? "驱动模式" : "无驱动模式") + "\n");
        ...
        new Thread(new Runnable() { public void run() {
            try {
                ...
                if (driverMode) {
                    post("检查驱动...\n");
                    String match = RunnerSupport.findMatchingDriverEntryName(driverZipFile);
                    if (match == null || match.length() == 0) {
                        post("未找到当前内核对应驱动: " + RunnerSupport.getKernelVersion() + "\n");
                        return;
                    }
                    matchedDriverName = new File(match).getName();
                    post("刷入驱动: " + matchedDriverName + "\n");
                    RunnerSupport.installDriverFromZip(MainActivity.this, driverZipFile);
                    post("驱动完成\n");
                    final String autoChoice = getAutoDriverChoiceForWePro();
                    if (autoChoice != null) {
                        post("AuraKernel.sh 自动识别驱动类型：" + describeAutoDriverChoice(autoChoice) + "，启动后将自动回车输入 " + autoChoice + "\n");
                    }
                } else {
                    post("无驱动模式：跳过驱动匹配和刷入\n");
                }
                ...
                if (selectedFile.canRead()) {
                    ...
                    rootPath = RunnerSupport.prepareRootExecutableFromPath(selectedFile.getAbsolutePath(), selectedName);
                    isElf = RunnerSupport.looksLikeElfOrBinaryName(selectedName);
                }

                activeRootPath = rootPath;
                post("Root 运行路径: " + rootPath + "\n");
                post("启动真实终端，开始读取 stdout/stderr...\n");
                post("如果设备支持 script，将使用 PTY 伪终端；否则自动回退普通管道。\n\n");
    ...
    private void startInteractiveRootProcess(String rootPath, boolean isElf) throws Exception {
        ...
        handler.post(new Runnable() { public void run() { updateInputState(); } });

        post("[AuraKernel] root command dispatched\n");
        processWriter.write(command);
        processWriter.newLine();
        processWriter.flush();
        scheduleAutoDriverChoiceIfNeeded();
    ...
    private String getAutoDriverChoiceForWePro() {
        ...
        if (selectedName == null || !"AuraKernel.sh".equalsIgnoreCase(selectedName.trim())) return null;
    ...
    private void autoSendLine(String value, String reason) {
        ...
        try {
            ...
            processWriter.newLine();
            processWriter.flush();
            append("\n[自动输入] " + reason + "：" + (value == null ? "" : value) + " + 回车\n");
        } catch (Exception e) {
    ...
    private void updateRunButton() {
        ...
        if (driverMode) ok = ok && driverZipFile != null && matchedDriverName.length() > 0;
        runButton.setEnabled(ok);
        runButton.setAlpha(ok ? 1f : 0.55f);
        runButton.setText(running ? "运行中..." : (driverMode ? "刷入并运行" : "直接运行"));
    ...
    private void showFilePicker(String title, boolean zipOnly, String lastDirPref, FilePickCallback callback) {
    ...
    private View buildBrowserView(final BrowserState state) {
        ...
        close.setOnClickListener(new View.OnClickListener() { public void onClick(View v) { state.dialog.dismiss(); } });
        ...
        addBrowserAction(actions, "上级", new View.OnClickListener() { public void onClick(View v) {
            File parent = state.currentDir.getParentFile();
            if (parent != null) navigateBrowser(state, parent);
        }});
        addBrowserAction(actions, "/sdcard", new View.OnClickListener() { public void onClick(View v) {
            navigateBrowser(state, Environment.getExternalStorageDirectory());
        }});
        addBrowserAction(actions, "根目录 /", new View.OnClickListener() { public void onClick(View v) {
            navigateBrowser(state, new File("/"));
        }});
        addBrowserAction(actions, "收藏当前", new View.OnClickListener() { public void onClick(View v) {
        ...
        for (int i = 0; i < favs.size(); i++) {
            ...
            addBrowserAction(actions, shortPath(f.getAbsolutePath()), new View.OnClickListener() { public void onClick(View v) { navigateBrowser(state, f); } });
        }
        ...
        if (files.size() == 0) {
            TextView empty = text("当前目录为空，或没有读取权限。可以尝试授权全部文件访问，或切换到 /sdcard、/storage/emulated/0、根目录 /。", 13, subTextColor(), Typeface.NORMAL);
            ...
            for (int i = 0; i < files.size(); i++) {
                ...
                row.setOnClickListener(new View.OnClickListener() { public void onClick(View v) {
                    if (f.isDirectory()) {
                        ...
                        getSharedPreferences(PREFS, MODE_PRIVATE).edit().putString(state.lastDirPref, state.currentDir.getAbsolutePath()).apply();
    ...
    private View browserRow(File f, boolean zipOnly) {
        ...
        TextView icon = text(f.isDirectory() ? "目录" : (zipOnly ? "ZIP" : "FILE"), 11, primaryColor(), Typeface.BOLD);
        icon.setGravity(Gravity.CENTER);
        ...
        TextView name = text(f.getName().length() == 0 ? f.getAbsolutePath() : f.getName(), 14, textColor(), f.isDirectory() ? Typeface.BOLD : Typeface.NORMAL);
        name.setSingleLine(true);
        name.setEllipsize(TextUtils.TruncateAt.MIDDLE);
        texts.addView(name, new LinearLayout.LayoutParams(-1, -2));
        String info = f.isDirectory() ? f.getAbsolutePath() : readableSize(f.length()) + " · " + f.getAbsolutePath();
    ...
    private List<File> listFilesForBrowser(File dir, boolean zipOnly) {
        ...
        File[] arr = null;
        try { arr = dir.listFiles(); } catch (Exception ignored) {}
    ...
    private void clearFavoriteDirs() {
        getSharedPreferences(PREFS, MODE_PRIVATE).edit().remove(PREF_FAV_DIRS).remove(PREF_LAST_RUN_DIR).remove(PREF_LAST_DRIVER_DIR).apply();
    ...
    private void requestStoragePermissionQuietly() {
        if (Build.VERSION.SDK_INT >= 23) {
            try {
                ...
                if (checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) perms.add(Manifest.permission.WRITE_EXTERNAL_STORAGE);
                if (perms.size() > 0) requestPermissions(perms.toArray(new String[perms.size()]), REQ_STORAGE);
            } catch (Exception ignored) {}
        }
    }
    ...
    private void openAllFilesAccessSettings() {
        try {
            ...
            catch (Exception ignored) { Toast.makeText(this, "无法打开授权页面", Toast.LENGTH_SHORT).show(); }
        }
    }
    ...
    private void joinGroupChat() {
        ...
        try {
            Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse("mqqapi://card/show_pslcard?src_type=internal&version=1&uin=" + groupNumber + "&card_type=group&source=qrcode"));
            startActivity(intent);
        } catch (Exception e) {
            try {
    ...
    private void post(final String s) {
        handler.post(new Runnable() { public void run() { append(s); } });
    }

    private void scrollTerminalBottom() {
        if (terminalScroll != null) {
            terminalScroll.post(new Runnable() { public void run() { terminalScroll.fullScroll(View.FOCUS_DOWN); } });
        }
    }
    ...
    private int bgColor() { return nightMode ? Color.rgb(10, 13, 20) : Color.rgb(250, 251, 254); }
    private int cardColor() { return nightMode ? Color.rgb(22, 26, 36) : Color.WHITE; }
    private int textColor() { return nightMode ? Color.rgb(238, 242, 248) : Color.rgb(17, 23, 34); }
    private int subTextColor() { return nightMode ? Color.rgb(144, 153, 169) : Color.rgb(112, 120, 136); }
    private int borderColor() { return nightMode ? Color.rgb(42, 48, 62) : Color.rgb(235, 240, 248); }
    private int primaryColor() { return Color.rgb(22, 119, 255); }
    private int tagColor() { return nightMode ? Color.rgb(26, 44, 75) : Color.rgb(239, 245, 255); }
    private int terminalBgColor() { return Color.rgb(9, 12, 18); }
    private int successColor() { return nightMode ? Color.rgb(79, 224, 146) : Color.rgb(12, 162, 92); }
    private int successBgColor() { return nightMode ? Color.rgb(18, 55, 38) : Color.rgb(229, 250, 239); }
    private int dangerColor() { return nightMode ? Color.rgb(255, 139, 148) : Color.rgb(213, 42, 55); }
    private int dangerBgColor() { return nightMode ? Color.rgb(66, 30, 38) : Color.rgb(255, 238, 240); }
    private int disabledColor() { return nightMode ? Color.rgb(37, 43, 56) : Color.rgb(224, 229, 238); }
