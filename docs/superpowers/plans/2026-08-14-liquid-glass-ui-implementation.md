...
### 阶段 0：代码侦察（1 个任务）
- [x] T0 精读 MainActivity.java 关键区段，输出"UI 构建点清单"：
  - 根布局结构与层级（背景色、各页面容器）
...
### 阶段 1：核心库（2 个任务，无依赖）
- [x] T1 `ui/core/FluidPhysics.java` —— 纯 Java 数学库（无 Android 依赖，可 JVM 单测）：
...
- [x] T2 `ui/core/ThemeManager.java` —— 单例：
...
### 阶段 2：流体背景（2 个任务）
- [x] T3 `ui/LiquidBackgroundView.java` —— 全屏自绘 View：
...
- [x] T4 接入 MainActivity：根布局最底层挂 LiquidBackgroundView，页面容器改为透明
- 验证：真机/模拟器目视（波浪无缝流动、切后台暂停、主题切换颜色平滑过渡）

### 阶段 3：玻璃组件（2 个任务）
- [x] T5 `ui/GlassCard.java` —— 自绘圆角玻璃容器（Layout 包装，子 View 内容不变）：
...
- [x] T6 `ui/GlassButton.java` —— 渐变底 + 高光 + 按压 scale 0.93 回弹（cubic-bezier(.34,1.8,.64,1)），替换现有主要按钮
- 验证：目视 + 按压手感

### 阶段 4：LiquidTabBar（1 个任务）
- [x] T7 `ui/LiquidTabBar.java`（基于现有 CapsuleNavBar 改造，保留接口）：
  - 玻璃底栏 + 4 个 tab（图标+文字）
...
### 阶段 5：流体进度条（1 个任务）
- [x] T8 `ui/FluidProgressView.java`：
...
### 阶段 6：壁纸（1 个任务）
- [x] T9 `ui/core/WallpaperStore.java`：
...
- [x] T11 "我的"页面新增设置区：主题色系切换（绿/青蓝/紫粉）、明暗手动覆盖、壁纸选择入口、当前主题预览
- 验证：6 套主题下所有页面颜色一致；功能全量冒烟

### 阶段 8：极致交互（1 个任务）
- [x] T12 下拉拉扯感：内容区 overscroll 时背景波浪被向下拉伸（波浪基线上移 + 振幅增大），松手弹簧回弹
- 验证：各页面下拉手感统一、不干扰正常滚动

### 阶段 9：性能与回归（2 个任务）
- [x] T13 性能收尾：帧率自适应验证（低端机降层）、RenderEffect 降级路径在 API 26 模拟器验证、内存泄漏检查（切后台/切主题循环 20 次）
- [x] T14 全量回归（spec §10 测试计划 6 项）+ 修复

## 提交策略

每个任务完成后单独 commit（`feat(liquid): <task>`），保持主分支随时可构建。

## 状态：全部任务已完成（2026-08-14，release APK 构建通过 2.3MB）
